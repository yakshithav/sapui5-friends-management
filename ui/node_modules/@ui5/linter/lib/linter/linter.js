...
export async function lintProject({ rootDir, filePatterns, ignorePatterns, coverage, details, fix, configPath, ui5Config, noConfig, }, sharedLanguageService) {
...
export async function lintFile({ rootDir, filePatterns, ignorePatterns, namespace, coverage, details, fix, configPath, noConfig, }, sharedLanguageService) {
    let config = {};
    if (noConfig !== true) {
        const configMngr = new ConfigManager(rootDir, configPath);
        config = await configMngr.getConfiguration();
...
async function lint(resourceReader, options, config, sharedLanguageService) {
    ...
    const res = await lintWorkspace(workspace, filePathsWorkspace, options, config, matchedPatterns, sharedLanguageService);
    checkUnmatchedPatterns(filePatterns, matchedPatterns);
    lintEnd();
    return res;
}
async function getProjectGraph(rootDir, ui5Config) {
    ...
    if (typeof ui5Config === "object") {
        ...
        if (ui5Config)
            throw new Error(`Unable to find UI5 config file '${ui5Config}'`);
    ...
    if (!rootConfigPath && !rootConfiguration) {
        throw new Error(`Unable to find a UI5 project at ${rootDir}. \n` +
...
function createProjectConfig(projectType, projectSrcPath, projectTestPath) {
    let resourcesConfig = {
        configuration: {
            paths: {},
        },
...
async function fileExists(dirPath) {
    const stats = await fsStat(dirPath);
    return stats && stats.isFile();
}
function sortLintResults(lintResults) {
    lintResults.sort((a, b) => a.filePath.localeCompare(b.filePath));
}
function isFileIncluded(file, patterns, patternsMatch) {
    let include = true;
...
export function resolveReader({ patterns, resourceReader, inverseResult = false, patternsMatch, fsToVirtualPathOptions, }) {
    if (!patterns.length) {
    ...
    return createFilterReader({
        ...
        callback: (resource) => {
            ...
            const resPath = transformVirtualPathToFilePath(resource.getPath(), fsToVirtualPathOptions);
 ...
 */
function checkUnmatchedPatterns(patterns, patternsMatch) {
...
export function mergeIgnorePatterns(options, config) {
    return [
        ...(config.ignores ?? []),
