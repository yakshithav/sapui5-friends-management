...
export default async function lintWorkspace(workspace, filePathsWorkspace, options, config, patternsMatch, sharedLanguageService) {
    ...
    const lintContext = await runLintWorkspace(workspace, filePathsWorkspace, options, config, patternsMatch, libraryDependencies, sharedLanguageService);
    if (!options.fix) {
    ...
    for (let autofixIterations = 0; autofixIterations <= 10; autofixIterations++) {
        ...
        const autofixContext = await runAutofix(workspace, filePathsWorkspace, options, config, patternsMatch, libraryDependencies, sharedLanguageService, lastContext, autofixDryRun);
        if (!autofixContext) {
...
async function runAutofix(workspace, filePathsWorkspace, options, config, patternsMatch, libraryDependencies, sharedLanguageService, context, dryRun) {
    const rawLintResults = context.generateRawLintResults();
    const autofixResources = new Map();
    const preAutofixParsingErrors = new Map();
    ...
    if (autofixResult.size > 0) {
        ...
        context = await runLintWorkspace(workspace, filePathsWorkspace, optionsAfterFix, config, patternsMatch, libraryDependencies, sharedLanguageService);
        for (const { filePath, rawMessages } of autofixContext.generateRawLintResults()) {
            ...
            rawMessages.forEach((msg) => {
                if (msg.id === MESSAGE.AUTOFIX_ERROR) {
                    context.addLintingMessage(filePath, msg.id, msg.args);
                }
                if (msg.id === MESSAGE.PARSING_ERROR) {
                    const parsingError = msg;
...
async function runLintWorkspace(workspace, filePathsWorkspace, options, config, patternsMatch, libraryDependencies, sharedLanguageService) {
...
async function getLibraryDependenciesFromManifest(workspace, virBasePath) {
