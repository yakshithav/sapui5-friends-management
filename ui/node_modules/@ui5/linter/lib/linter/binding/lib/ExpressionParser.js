...
function FUNCTION_CALL(fnLeft, aArguments, aParts) {
    ...
    return clean(fnLeft(aParts, oReference).apply(oReference.base, aArguments.map(function (fnArgument) {
...
function PROPERTY_ACCESS(fnLeft, fnName, aParts, oReference) {
    var oParent = fnLeft(aParts), sIdentifier = fnName(aParts), // BEWARE: evaluate propertyNameValue AFTER baseValue!
...
function tokenize(fnResolveBinding, sInput, iStart) {
    ...
    aTokens = [], oTokenizer = new JSTokenizer();
    /**
        ...
        if (sExpectedTokenId) {
            if (!oToken) {
                ...
                error("Expected " + sExpectedTokenId + " but instead saw "
                    + sInput.slice(oToken.start, oToken.end), sInput, oToken.start + 1);
            }
        }
        iNextToken += 1;
        return oToken;
