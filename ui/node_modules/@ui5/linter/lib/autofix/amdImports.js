...
import { ChangeAction } from "./autofix.js";
...
function resolveRelativeDependency(dependency, moduleName) {
...
export function hasBody(moduleDeclaration) {
...
export function getFactoryBody(moduleDeclaration) {
...
export function getDependencies(moduleDeclaration, resourcePath) {
    const dependencies = new Map(); // Module name to identifier
    ...
    const moduleName = resourcePath.startsWith("/resources/") ? resourcePath.substring("/resources/".length) : undefined;
...
export function createDependencyInfo(dependencyArray, resourcePath) {
    const moduleName = resourcePath.startsWith("/resources/") ? resourcePath.substring("/resources/".length) : undefined;
        ...
        for (const child of syntaxList.getChildren()) {
            if (child.kind === ts.SyntaxKind.CommaToken) {
                if (previousDependency) {
                    previousDependency.trailingComma = child;
...
function getParameterSyntax(node) {
    let syntaxList;
    let hasParens = false;
...
export function addDependencies(defineCall, moduleDeclarationInfo, changeSet, resourcePath, removedDependencies) {
    ...
    const depsSeparator = extractIdentifierSeparator(dependencies?.[1]?.getFullText() ?? dependencies?.[0]?.getFullText() ?? "");
    const identifiersSeparator = extractIdentifierSeparator(factory.parameters[1]?.getFullText() ??
        ...
        if (newDependencyValue) {
            ...
            if (insertAfterDependencyElement || (dependencies && insertDependenciesAfterIndex === -1)) {
                ...
                changeSet.push({
                    action: ChangeAction.INSERT,
                    start,
                    value: `[${formatDependencies(newDependencyValue, depsSeparator.trailing, { pos: start, node: defineCall })}], `,
                });
            }
        }
...
export function removeDependencies(dependenciesToRemove, moduleDeclarationInfo, changeSet, resourcePath, declaredIdentifiers) {
    ...
    [...dependencyMap]
        ...
        if (index >= 0 && dependencies?.elements[index]) {
            ...
            ts.factory.updateArrayLiteralExpression(dependencies, ts.factory.createNodeArray(newElements));
            // Update the source
            let start = existingDependency.node.getFullStart();
            let end = existingDependency.node.getEnd();
            if (existingDependency.leadingComma) {
        ...
        if (index >= 0 &&
            ...
            if (ts.isFunctionExpression(factory)) {
                ts.factory.updateFunctionExpression(factory, factory.modifiers, factory.asteriskToken, factory.name, factory.typeParameters, ts.factory.createNodeArray(newParams), factory.type, factory.body);
            }
            else if (ts.isFunctionDeclaration(factory)) {
                ts.factory.updateFunctionDeclaration(factory, factory.modifiers, factory.asteriskToken, factory.name, factory.typeParameters, ts.factory.createNodeArray(newParams), factory.type, factory.body);
            }
            else if (ts.isArrowFunction(factory)) {
                ts.factory.updateArrowFunction(factory, factory.modifiers, factory.typeParameters, ts.factory.createNodeArray(newParams), factory.type, factory.equalsGreaterThanToken, factory.body);
            }
...
function extractIdentifierSeparator(input) {
...
function formatDependencies(input, depsSeparator, posInfo) {
...
//# sourceMappingURL=amdImports.js.map