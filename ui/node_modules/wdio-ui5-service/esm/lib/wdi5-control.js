...
export class WDI5Control {
    ...
    _webElement = null; // TODO: type "org.openqa.selenium.WebElement"
    // wdio element retrieved separately via $()
    _webdriverRepresentation = null;
    _metadata = {};
    // TODO: move to _metadata
    _wdio_ui5_key = null;
    _generatedUI5Methods;
    _initialisation = false;
    _forceSelect = false;
    _logging;
    _wdioBridge = {};
    _generatedWdioMethods;
    _domId;
    _browserInstance;
    constructor(oOptions) {
        const { browserInstance, controlSelector, wdio_ui5_key, forceSelect, generatedUI5Methods, webdriverRepresentation, webElement, domId } = oOptions;
     ...
     */
    async _interactWithControl(oOptions) {
     ...
     * @returns
     */
    async _renewWebElement(id = this._domId) {
        if (this._domId) {
    ...
    _attachWdioControlBridge(sReplFunctionNames) {
        // check the validity of param
        if (sReplFunctionNames) {
            sReplFunctionNames.forEach(async (sMethodName) => {
     ...
     */
    async _executeControlMethod(methodName, webElement = this._webElement, ...args) {
            ...
            case "aggregation": // also applies for getAggregation convenience methods such as $ui5control.getItems()
                ...
                if ((args.length > 0 && typeof args[0] === "boolean" && args[0] === false) || args.length === 0) {
                    ...
                    if (args[0] <= result.result.length) {
                        ...
                        console.error(`tried to get an control at index: ${args[0]} of an aggregation outside of aggregation length: ${result.result.length}`);
                    }
                }
                else {
     ...
     * @return {any}
     */
    async _getAggregation(aggregationName, webElement = this._webElement) {
        const _forceSelect = util.types.isProxy(this._forceSelect)
            ? await Promise.resolve(this._forceSelect)
            : this._forceSelect;
        const _logging = util.types.isProxy(this._logging)
     ...
     */
    async _renewWebElementReference(isRefresh = false) {
        if (this._domId) {
            //> REVISIT: browser.allControls uses this._domId for selection
            const newWebElement = (await this._getControl(isRefresh ? this._controlSelector : { selector: { id: this._domId } })).domElement; // added to have a more stable retrieval experience
     ...
     */
    async _getControl(controlSelector = this._controlSelector) {
        ...
        if (_result.status === 0 && !_result.domElement[ELEMENT_KEY]) {
            const elementReference = (await this._browserInstance.execute((id) => {
                const webElement = document.evaluate(`//*[@id='${id}']`, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
                return webElement;
            }, _result.id));
