...
export async function setup(config) {
    ...
    if (browser.isMultiremote) {
        ;
        browser.instances.forEach((name) => {
 ...
 */
export async function injectUI5(config, browserInstance) {
    if (!config?.wdi5) {
...
function _createWdioUI5KeyFromSelector(selector) {
    ...
    const wdi5_ui5_key = `${orEmpty(_selector.id)}${orEmpty(_selector.viewName)}${orEmpty(_selector.controlType)}${orEmpty(JSON.stringify(_selector.bindingPath))}${orEmpty(JSON.stringify(_selector.i18NText))}${orEmpty(JSON.stringify(_selector.descendant))}${orEmpty(JSON.stringify(_selector.labelFor))}${orEmpty(JSON.stringify(_selector.properties))}${orEmpty(JSON.stringify(_selector.ancestor))}`.replace(/[^0-9a-zA-Z]+/, "");
    return wdi5_ui5_key;
}
/**
...
export async function _addWdi5Commands(browserInstance) {
    ...
    browser.addCommand("asObject", async (_uuid) => {
        const _result = (await clientSide_getObject(_uuid));
...
async function _navTo(sComponentId, sName, oParameters, oComponentTargetInfo, bReplace, browserInstance) {
    const result = (await clientSide__navTo(sComponentId, sName, oParameters, oComponentTargetInfo, bReplace, browserInstance));
    if (result.status === 1) {
