    ...
    create: function(context) {
        ...
        function isValid(node) {
            if (node && node.arguments && node.arguments.length > 0) {
                var target = getProperty(node.arguments[0], "target");
                if (target) {
                    // get property target from first argument, get property shellHash from property target
                    var shellHash = getProperty(target, "shellHash");
                    // check if property shellHash has value '#' or '#Shell-home' or ""
                    if (shellHash && getName(shellHash) === "#Shell-home" || getName(shellHash) === "#" || getName(shellHash) === "") {
                        return true;
                    }
                    var semanticObject = getProperty(target, "semanticObject");
                    var action = getProperty(target, "action");
